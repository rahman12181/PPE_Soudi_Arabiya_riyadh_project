// ignore_for_file: unused_local_variable, deprecated_member_use, unused_field
...
class _LeaveRequestState extends State<LeaveRequest> with TickerProviderStateMixin {
  ...
  Future<void> _loadEmployeeData() async {
    ...
    setState(() => empCodeCtrl.text = savedEmpCode);
    if (profileData != null) {
      try {
        final data     = jsonDecode(profileData);
        final fullName = data["full_name"] ?? "";
        setState(() => empNameCtrl.text = fullName);
      } catch (_) {}
    }
  }
  ...
  void _showSuccessPopup(String message) {
    ...
    setState(() { _successMessage = message; _showSuccessDialog = true; _showErrorDialog = false; });
  }

  void _showErrorPopup(String message) {
    HapticFeedback.mediumImpact();
    setState(() { _errorMessage = message; _showErrorDialog = true; _showSuccessDialog = false; });
  }

  void _closeDialogs() {
    HapticFeedback.lightImpact();
    setState(() { _showSuccessDialog = false; _showErrorDialog = false; });
  }
  ...
  Future<void> _submitLeave() async {
    if (!_formKey.currentState!.validate()) { HapticFeedback.mediumImpact(); return; }
    if (selectedLeaveType == null) { _showErrorPopup("Please select leave type"); return; }

    if (!await _hasInternet()) {
      _showErrorPopup("No internet connection. Please check your connection.");
      return;
    }
    ...
    try {
      ...
      if (fromParts.length != 3 || toParts.length != 3) {
        _showErrorPopup("Invalid date format. Please use DD-MM-YYYY."); return;
      }
      final fromDate = DateTime(int.parse(fromParts[2]), int.parse(fromParts[1]), int.parse(fromParts[0]));
      final toDate   = DateTime(int.parse(toParts[2]),   int.parse(toParts[1]),   int.parse(toParts[0]));
      if (toDate.isBefore(fromDate)) { _showErrorPopup("To date cannot be before from date."); return; }
    } catch (_) {
      _showErrorPopup("Invalid date format. Please use DD-MM-YYYY."); return;
    }
    ...
    try {
      ...
      if (result["success"] == true) {
        ...
        _showSuccessPopup(result["message"] ?? "Leave applied successfully!");
      } else {
        await Future.delayed(const Duration(milliseconds: 300));
  ...
  InputDecoration _inputDecoration(String label, {IconData? icon}) {
    ...
    return InputDecoration(
      labelText: label,
      labelStyle: TextStyle(fontSize: width * 0.035, color: isDarkMode ? Colors.grey[400] : Colors.grey[600], fontWeight: FontWeight.w500),
      floatingLabelStyle: const TextStyle(color: skyBlue, fontWeight: FontWeight.w600),
      contentPadding: EdgeInsets.symmetric(horizontal: width * 0.04, vertical: width * 0.04),
      border: OutlineInputBorder(borderRadius: BorderRadius.circular(width * 0.03), borderSide: BorderSide.none),
      enabledBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(width * 0.03),
        borderSide: BorderSide(color: isDarkMode ? Colors.grey[700]! : Colors.grey[300]!, width: 1.5),
      ),
      ...
      prefixIcon: icon != null ? Icon(icon, size: width * 0.05, color: skyBlue) : null,
      suffixIcon: label.contains("Date") ? Icon(Icons.calendar_today_rounded, size: width * 0.05, color: skyBlue) : null,
    );
  }
  ...
  Widget _buildDropdown({
    ...
    return Column(
      ...
      children: [
        Text(label, style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w600,
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(width * 0.03),
            border: Border.all(color: isDarkMode ? Colors.grey[700]! : Colors.grey[300]!, width: 1.5),
          ),
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              icon: Icon(Icons.keyboard_arrow_down_rounded, color: skyBlue, size: width * 0.06),
              items: options.map((opt) => DropdownMenuItem(
                value: opt,
                child: Row(children: [
                  Icon(icon, size: width * 0.045, color: skyBlue),
                  SizedBox(width: width * 0.03),
                  Text(opt, style: TextStyle(fontSize: width * 0.038, fontWeight: FontWeight.w500,
                      color: isDarkMode ? Colors.white : Colors.black)),
                ]),
              )).toList(),
              onChanged: _isLoading ? null : (v) { if (v != null) { HapticFeedback.selectionClick(); onChanged(v); } },
            ),
          ),
        ),
      ],
                                  ...
                                  child: Form(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Row(
                                          children: [
                                            Expanded(child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text("From Date", style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w600, color: textColor)),
                                                SizedBox(height: height * 0.01),
                                                TextFormField(
                                                  controller: fromDateCtrl, readOnly: true,
                                                  ...
                                                  validator:  (v) => v!.isEmpty ? "Please select from date" : null,
                                                ),
                                              ],
                                            )),
                                            SizedBox(width: width * 0.03),
                                            Expanded(child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text("To Date", style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w600, color: textColor)),
                                                SizedBox(height: height * 0.01),
                                                TextFormField(
                                                  controller: toDateCtrl, readOnly: true,
                                                  onTap:      () => selectDate(toDateCtrl),
                                                  style: TextStyle(fontSize: width * 0.04, color: textColor, fontWeight: FontWeight.w500),
                                                  decoration: _inputDecoration("DD-MM-YYYY", icon: Icons.calendar_month),
                                                  validator:  (v) => v!.isEmpty ? "Please select to date" : null,
                                                ),
                                              ],
                                            )),
                                          ],
                                        ...
                                        // Reason
                                        Text("Reason for Leave", style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w600, color: textColor)),
                                        SizedBox(height: height * 0.01),
                                        TextFormField(
                                          controller: reasonCtrl, maxLines: 4, minLines: 3,
                                          style: TextStyle(fontSize: width * 0.04, color: textColor, fontWeight: FontWeight.w500),
                                          decoration: _inputDecoration("Explain your reason...", icon: Icons.description_outlined),
                                          validator: (v) => v!.isEmpty ? "Please enter reason for leave" : null,
                                        ),
                                        ...
                                        Text("Incharge Replacement", style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w600, color: textColor)),
                                        SizedBox(height: height * 0.01),
                                        TextFormField(
                                        ...
                                        AnimatedContainer(
                                          ...
                                          child: ElevatedButton(
                                            ...
                                            style: ElevatedButton.styleFrom(
                                              backgroundColor: _isLoading ? Colors.grey.shade400 : skyBlue,
                                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(width * 0.035)),
                                              ...
                                              children: [
                                                AnimatedOpacity(
                                                  ...
                                                  child: Row(
                                                    mainAxisAlignment: MainAxisAlignment.center,
                                                    children: [
                                                      Icon(Icons.send_rounded, size: width * 0.05, color: Colors.white),
                                                      SizedBox(width: width * 0.02),
                                                      Text("Submit Leave Application",
                                                          style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 0.5)),
                                                    ],
                                                  ),
                                                ),
                                                if (_isLoading)
                                                  SizedBox(
                                                    ...
                                                    child: CircularProgressIndicator(strokeWidth: 3, color: Colors.white, backgroundColor: Colors.white.withOpacity(0.3)),
                                                  ),
                                              ],
                                            ),
                                          ),
                                        ...
                                        // Cancel button
                                        SizedBox(
                                          width: double.infinity, height: height * 0.055,
                                          child: TextButton(
                                            onPressed: _isLoading ? null : () { HapticFeedback.lightImpact(); Navigator.pop(context); },
                                            style: TextButton.styleFrom(
                                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(width * 0.035)),
                                              foregroundColor: subtitleColor,
                                            ),
                                            child: Text("Cancel", style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w600)),
                                          ),
                                        ),
                                        ...
                                        AnimatedContainer(
                                          ...
                                          child: Row(
                                            children: [
                                              Icon(Icons.info_outline_rounded, size: width * 0.05, color: skyBlue),
                                              SizedBox(width: width * 0.03),
                                              Expanded(
                                                child: Text(
                                                  "Leave application will be sent to your manager for approval",
                                                  style: TextStyle(fontSize: width * 0.035, color: textColor, fontWeight: FontWeight.w500),
                                                ),
                                              ),
                                            ],
                                          ),
                      ...
                      child: Column(
                        ...
                        children: [
                          AnimatedContainer(
                            ...
                            decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.green.withOpacity(0.1), border: Border.all(color: Colors.green, width: 3)),
                            child: Icon(Icons.check, size: width * 0.1, color: Colors.green),
                          ),
                          SizedBox(height: height * 0.03),
                          Text("Success!", style: TextStyle(fontSize: width * 0.06, fontWeight: FontWeight.bold, color: Colors.green)),
                          SizedBox(height: height * 0.015),
                          Text(_successMessage, style: TextStyle(fontSize: width * 0.04, color: textColor, height: 1.4), textAlign: TextAlign.center),
                          SizedBox(height: height * 0.03),
                          ScaleTransition(
                            scale: _pulseAnimation,
                            child: SizedBox(
                              width: double.infinity, height: height * 0.06,
                              child: ElevatedButton(
                                style: ElevatedButton.styleFrom(backgroundColor: Colors.green, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(width * 0.03))),
                                onPressed: _navigateToDashboard,
                                child: Text("Go to Dashboard", style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w700, color: Colors.white)),
                              ),
                            ),
                          ),
                          SizedBox(height: height * 0.01),
                          TextButton(
                            ...
                            child: Text("Apply Another Leave", style: TextStyle(color: skyBlue, fontSize: width * 0.038, fontWeight: FontWeight.w600)),
                          ),
                        ],
                      ),
                    ),
                      ...
                      child: Column(
                        ...
                        children: [
                          AnimatedContainer(
                            duration: const Duration(milliseconds: 500), curve: Curves.elasticOut,
                            width: width * 0.2, height: width * 0.2,
                            decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.red.withOpacity(0.1), border: Border.all(color: Colors.red, width: 3)),
                            child: Icon(Icons.error_outline, size: width * 0.1, color: Colors.red),
                          ),
                          SizedBox(height: height * 0.03),
                          Text("Error", style: TextStyle(fontSize: width * 0.06, fontWeight: FontWeight.bold, color: Colors.red)),
                          SizedBox(height: height * 0.015),
                          Text(_errorMessage, style: TextStyle(fontSize: width * 0.04, color: textColor, height: 1.4), textAlign: TextAlign.center),
                          SizedBox(height: height * 0.03),
                          SizedBox(
                            width: double.infinity, height: height * 0.06,
                            child: ElevatedButton(
                              style: ElevatedButton.styleFrom(backgroundColor: Colors.red, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(width * 0.03))),
                              onPressed: _closeDialogs,
                              child: Text("Try Again", style: TextStyle(fontSize: width * 0.04, fontWeight: FontWeight.w700, color: Colors.white)),
                            ),
                          ),
                          SizedBox(height: height * 0.01),
                          TextButton(
                            onPressed: _closeDialogs,
                            child: Text("Cancel", style: TextStyle(color: subtitleColor, fontSize: width * 0.038, fontWeight: FontWeight.w600)),
                          ),
                        ],
                      ),
                    ),
